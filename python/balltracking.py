...
class Ball:
    ...
    def tracking(self, frame, pts, args):
        ...
        if args == "red":
            ...
            bright_mask = cv2.inRange(hsv, bright_red_lower_bounds, bright_red_upper_bounds)

            dark_red_lower_bounds = (160, 100, 100)
            dark_red_upper_bounds = (179, 255, 255)
            dark_mask = cv2.inRange(hsv, dark_red_lower_bounds, dark_red_upper_bounds)

        elif args == "blue":
            bright_blue_lower_bounds = (110, 150, 150)  # (65, 60, 60)
            bright_blue_upper_bounds = (140, 255, 255)  # (80, 255, 100)
            bright_mask = cv2.inRange(hsv, bright_blue_lower_bounds, bright_blue_upper_bounds)

            dark_blue_lower_bounds = (130, 255, 255)  # (65, 60, 160)
            dark_blue_upper_bounds = (140, 255, 255)  # (80, 255, 179)
            dark_mask = cv2.inRange(hsv, dark_blue_lower_bounds, dark_blue_upper_bounds)

        elif args == "green":
            bright_green_lower_bounds = (65, 60, 60)
            bright_green_upper_bounds = (80, 255, 100)
            bright_mask = cv2.inRange(hsv, bright_green_lower_bounds, bright_green_upper_bounds)

            dark_green_lower_bounds = (65, 60, 160)
            dark_green_upper_bounds = (80, 255, 179)
            dark_mask = cv2.inRange(hsv, dark_green_lower_bounds, dark_green_upper_bounds)

        else:
            print("Error: wrong color")

        # after masking the red shades out, I add the two images
        weighted_mask = cv2.addWeighted(bright_mask, 1.0, dark_mask, 1.0, 0.0)
